...
public class Join2 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String optionName = optionFile.replaceAll("\\.csv$", "");
            String outputFile = "file2_" + optionName + ".csv";
            
            System.out.println("Starting INNER JOIN - using option file: " + optionFile);
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:duckdb:")) {
                ...
                String optionColumns = getColumns(conn, optionFile);
                String configColumns = getColumns(conn, "Csv_bom_esn_config.csv");
                ...
                Set<String> optionSerials = getSerials(conn, optionFile);
                Set<String> configSerials = getSerials(conn, "Csv_bom_esn_config.csv");
                ...
                try (Statement stmt = conn.createStatement()) {
                    stmt.execute("COPY (" +
                        ...
                        "l.shop_order_num, " +
                        "s.build_year " +
                        "FROM read_csv_auto('" + optionFile + "') l " +
                        "INNER JOIN read_csv_auto('Csv_bom_esn_config.csv') s " +
    ...
    private static String getColumns(Connection conn, String file) throws SQLException {
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT * FROM read_csv_auto('" + file + "') LIMIT 0")) {
    ...
    private static Set<String> getSerials(Connection conn, String file) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT Engine_serial FROM read_csv_auto('" + file + "') LIMIT 1000")) {
            while (rs.next()) {
