// Join.java - HYBRID VERSION (works with current directory and data/ folder)
...
public class Join {
    ...
    public static void main(String[] args) {
        try {
            ...
            // ===== FIND child_part_option.csv in current OR data/ directory =====
            String childPartCsv = findFile("child_part_option.csv");
            ...
            if (childPartCsv != null) {
                System.out.println("[CONVERT] Converting " + childPartCsv + " to Parquet (with headers)...");
                try (Connection conn = DriverManager.getConnection("jdbc:duckdb:engine_data.db")) {
                    try (Statement stmt = conn.createStatement()) {
                        ...
                        String csvPath = childPartCsv.replace("\\", "/");
                        stmt.execute("COPY (SELECT * FROM read_csv_auto('" + csvPath + "', header=true)) " +
                ...
                System.err.println("ERROR: child_part_option.csv not found in current directory or data/ folder!");
                System.exit(1);
            }
            ...
            // ***** MAIN JOIN LOGIC *****
            try (Connection conn = DriverManager.getConnection("jdbc:duckdb:engine_data.db")) {
                // Set optimal settings - EXACTLY MATCH PYTHON
                try (Statement stmt = conn.createStatement()) {
    ...
    /**
     * Find file in current directory or common folder
     */
    private static String findFile(String filename) {
        // Check current directory first
        File currentFile = new File(filename);
        if (currentFile.exists()) {
            System.out.println("[INFO] Found " + filename + " in current directory");
            return filename;
        }
        ...
        if (commonFile.exists()) {
            System.out.println("[INFO] Found " + filename + " in common folder: " + commonPath);
            return commonPath;
        }
        
        // Check with "data/" prefix
        String altPath = "data/" + filename;
        File altFile = new File(altPath);
        if (altFile.exists()) {
            System.out.println("[INFO] Found " + filename + " in common folder: " + altPath);
            return altPath;
        }
        
        System.out.println("[WARNING] " + filename + " not found in current directory or common folder");
        return null;
    }
}